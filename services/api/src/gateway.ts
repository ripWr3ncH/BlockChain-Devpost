 ...
 *
 * The receipt is not decoration. §3.8 step 4 says Central Authority's endorsement
...
function endorsingOrgs(channel: ChannelName): string[] {
  switch (channel) {
    case 'commitment':
    case 'exposure':
      return ['originating bank peer', 'CentralAuthorityMSP peer'];
    case 'claims':
      return ['BankAMSP peer', 'CentralAuthorityMSP peer'];
  }
}
