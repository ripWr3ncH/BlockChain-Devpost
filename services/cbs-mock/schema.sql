...
CREATE TABLE IF NOT EXISTS cbs.borrower (
    ...
    -- CIB subject code. §3.7.2 proposes Central Authority expose a pseudonymous
...
CREATE TABLE IF NOT EXISTS cbs.loan_master (
    ...
    principal_minor_unit   NUMERIC(20) NOT NULL CHECK (principal_minor_unit >= 0),
    outstanding_minor_unit NUMERIC(20) NOT NULL CHECK (outstanding_minor_unit >= 0),
    tenor_months       INTEGER     NOT NULL,
    -- Regulation B classification tier, as the bank itself assigns it.
    ...
    collateral_ref     TEXT,
    collateral_value_minor_unit NUMERIC(20),
...
CREATE TABLE IF NOT EXISTS cbs.deposit_account (
    ...
    balance_minor_unit   NUMERIC(20) NOT NULL CHECK (balance_minor_unit >= 0),
...
CREATE TABLE IF NOT EXISTS cbs.cl1_filing (
    ...
    reported_outstanding_minor_unit NUMERIC(20) NOT NULL,
    filed_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE (institution_msp, reference_date, loan_account_no)
);

COMMENT ON TABLE cbs.cl1_filing IS
    'The quarterly return as submitted to Central Authority. Verity reconciles against it; '
