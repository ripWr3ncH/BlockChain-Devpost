...
export default function DepositorPortal(): React.ReactNode {
  ...
  const taka = session
    ? (Number(session.balanceMinorUnit) / 100).toLocaleString('en-BD', { minimumFractionDigits: 2 })
    : '';
  ...
  const sign = async (): Promise<void> => {
    ...
    const digest = await leafDigest(session.accountRef, session.balanceMinorUnit, session.period);
        ...
        <>
          ...
          <div className="card">
            ...
            <dl className="receipt">
              ...
              <dt>{t.balance}</dt>
              <dd style={{ fontSize: '1.15rem', fontWeight: 700 }}>${taka}</dd>
              <dt>{t.period}</dt>
          ...
          <div className="card" style={{ marginTop: '1rem', opacity: both ? 1 : 0.45 }}>
            ...
            {session.priorityClass !== 'PROTECTED' && (
              <p className="hint">
                Above the $2,00,000 ceiling of the deposit protection scheme — which covers roughly
