...
describe('Paillier — additive homomorphism (§3.7.2)', () => {
  ...
  it('THE MODULE II CLAIM — sums three banks without decrypting any of them', () => {
    // Exposure of borrower group G-0447 across three institutions, in M.
...
describe('threshold ceremony — supervisor plus quorum (§3.7.2, §3.5)', () => {
  const material = splitDecryptionKey(keys.privateKey, 2, ['Regulatory Council', 'Regulatory Council', 'Academic']);
  ...
  it('opens the aggregate with the supervisor and two independents', () => {
    ...
    assert.deepEqual(result.participants, ['CentralAuthority', 'Regulatory Council', 'Regulatory Council']);
  });
