 ...
 *
 * Whitepaper §3.7.2:
 *   "Verity therefore decrypts each group total X_g to Central Authority, in a
 ...
 *     c  ≟  (1 + m·n) · rⁿ   (mod n²)
 *
 * That check is deterministic, so it runs in chaincode. Central Authority cannot
...
export interface CeremonyKeyMaterial {
  /** Held by Central Authority. Mandatory in every ceremony. */
 ...
 *
 * §3.7.2's "supervisor plus a quorum": the default here is Central Authority plus
 * 2 of 3 independents, matching the demo script.
 */
export function splitDecryptionKey(
  sk: PaillierPrivateKey,
  threshold = 2,
  holderNames: string[] = ['Regulatory Council', 'Regulatory Council', 'Academic independent'],
): CeremonyKeyMaterial {
...
export function runCeremony(
  ...
  if (!supervisorShare) {
    throw new Error(
      'CEREMONY_SUPERVISOR_ABSENT: threshold decryption requires Central Authority\'s share; ' +
  ...
  return {
    ...
    participants: ['CentralAuthority', ...[...distinct.values()].map((s) => s.holder)],
  };
}
