...
export interface SeedDepositor {
  accountRef: string;
  institutionMsp: string;
  balanceMinorUnit: string;
  priorityClass: 'PROTECTED' | 'ORDINARY_DEPOSITOR';
}

export interface SeedGroup {
  groupToken: string;
  /** Per-institution exposure, in millions of the unit currency. Module II sums these under encryption. */
...
/** Statutory classification reference dates, Regulation B (council-configured calendar). */
...
export function generate(options: GenerateOptions = {}): SeedData {
  const seed = options.seed ?? 'verity-midnight-2026';
...
function ordinaryLoan(rng: Rng, institutionMsp: string, index: number, groupTokens: string[]): SeedLoan {
  ...
  const commitmentId = `LN-${bankCode}${String(index + 1000).padStart(5, '0')}`;

  // Most exposures are small; a long tail is large. Log-uniform, in millions.
  const millions = Math.round(Math.exp(rng.float(Math.log(0.5), Math.log(300))) * 100) / 100;
  const outstanding = Math.round(millions * 1_000_000);
...
function plantedEvergreening(rng: Rng, institutionMsp: string, index: number): SeedLoan {
  const bankCode = institutionMsp === 'BankAMSP' ? 'A' : 'B';
  const commitmentId = `LN-${bankCode}E${String(index + 1).padStart(4, '0')}`;
  const millions = rng.float(40, 220);
  ...
  return {
    commitmentId,
    institutionMsp,
    outstanding: Math.round(millions * 1_000_000),
...
/**
 * §3.7.4: the deposit protection scheme covers up to $20,000 per depositor per
 * institution, "fully covering roughly 93% of bank accounts. What it leaves
 * open is the balances above $20,000, where most deposit VALUE sits."
 *
...
function depositor(rng: Rng, institutionMsp: string, index: number): SeedDepositor {
  ...
  const amount = protectedAccount
    ? rng.float(500, 20_000)
    : Math.exp(rng.float(Math.log(20_000), Math.log(1_500_000)));

  return {
    accountRef: `acct-${bankCode}-${String(index).padStart(5, '0')}`,
    institutionMsp,
    balanceMinorUnit: String(Math.round(amount * 100)),
...
function band(millions: number): string {
  if (millions < 1) return 'Under $1M';
  if (millions < 10) return '$1M-10M';
  if (millions < 50) return '$10M-50M';
  if (millions < 100) return '$50M-100M';
  if (millions < 150) return '$100M-150M';
  return 'Over $150M';
}
