...
#  event needs the bank's peer AND Central Authority's, so regulatory endorsement
...
# Central Authority and that institution's auditor. Without --collections-config
...
case "$CC_NAME" in
  commitment)
    CHANNEL=commitment
    # bank AND supervisor
    POLICY="AND(OR('BankAMSP.peer','BankBMSP.peer'),'CentralAuthorityMSP.peer')"
  ...
  claims)
    CHANNEL=claims
    POLICY="AND('BankAMSP.peer','CentralAuthorityMSP.peer')"
    ORGS="banka bb frc"
    ;;
...
declare -A ORG_MSP=(
  [banka]=BankAMSP [bankb]=BankBMSP [bb]=CentralAuthorityMSP [frc]=RegulatoryCouncilMSP
)
declare -A ORG_DOMAIN=(
  [banka]=banka.verity.local [bankb]=bankb.verity.local [bb]=bb.verity.local [frc]=frc.verity.local
)
declare -A ORG_PORT=(
...
ORDERER_CA="${NET}/organizations/ordererOrganizations/ord-bb.verity.local/orderers/orderer0.ord-bb.verity.local/msp/tlscacerts/tlsca.ord-bb.verity.local-cert.pem"
...
for org in $ORGS; do
  ...
  approve_log="$(peer lifecycle chaincode approveformyorg \
    -o "$ORDERER" --ordererTLSHostnameOverride orderer0.ord-bb.verity.local \
