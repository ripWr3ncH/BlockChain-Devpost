  ...
  for (const loan of seed.loans) {
    await client.query(
      `INSERT INTO cbs.loan_master
         (loan_account_no, borrower_id, institution_msp, sanction_date, principal_minor_unit,
          outstanding_minor_unit, tenor_months, classification, rs_counter,
          collateral_ref, collateral_value_minor_unit, sanctioning_officer, sanctioning_seniority)
  ...
  for (const d of seed.depositors) {
    await client.query(
      `INSERT INTO cbs.deposit_account
         (account_no, institution_msp, holder_name, holder_nid, balance_minor_unit, account_type, opened_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`,
      [
        d.accountRef,
        d.institutionMsp,
  ...
  for (const loan of seed.loans) {
    ...
    await client.query(
      `INSERT INTO cbs.cl1_filing
         (institution_msp, reference_date, loan_account_no, reported_classification, reported_outstanding_minor_unit)
       VALUES ($1,$2,$3,$4,$5)`,
