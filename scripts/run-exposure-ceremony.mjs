 ...
 *    decrypts each group total X_g to Central Authority, in a ceremony that needs
 ...
 *
 *   1. Central Authority opens a period by publishing the Paillier public key
...
/** §5.2 — $18,24,668M outstanding across the system, in M. */
const SYSTEM_CAPITAL = '2500';
const THETA = 0.25; // single-borrower limit, 25% of capital (Regulation C)

/**
 * The nominee structure from §1.1: comfortably inside every single-bank limit,
 * over the line once summed. 25% of $2,500M is 625 — no leg breaches it.
 */
const EXPOSURES = [
  { identity: 'officer-rahim', bank: 'BankAMSP', M: 520n },
  { identity: 'officer-shirin', bank: 'BankBMSP', M: 430n },
];
...
// ── 1 ──────────────────────────────────────────────────────────────────────
console.log('  1. Central Authority opens the period');
...
for (const e of EXPOSURES) {
  const ciphertext = paillier.encrypt(keys.publicKey, e.M);
  ...
  console.log(
    `     ${e.bank.replace('MSP', '').padEnd(6)} $${String(e.M).padStart(4)} M  →  ` +
      `ciphertext ${ciphertext.length} digits, block ${r.body.receipt.blockNumber}`,
  );
}
console.log(`     ${C.dim}each leg is below 25% of $${SYSTEM_CAPITAL} M capital — none breaches its own limit${C.off}\n`);
...
console.log(`     total $${opened.plaintext} M`);
...
console.log(`     $${record.total} M  vs  θ·C_system = $${record.threshold} M`);
console.log(
