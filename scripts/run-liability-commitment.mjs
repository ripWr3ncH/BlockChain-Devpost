...
const candidates = accounts.map((a) => {
  ...
  const balance = BigInt(a.balanceMinorUnit);
...
// ── 4 ──────────────────────────────────────────────────────────────────────
// Pick a depositor above the $2 00,000 protection ceiling — §3.7.4's point is
...
r = await call('/claims', 'officer-rahim', 'POST', {
  ...
  schedule: 'Payout within 17 working days of a resolution event (deposit protection scheme)',
});
if (r.status >= 400) fail('issuing the claim', r);
console.log(`     ${claimId}  $${(Number(subject.balance) / 100).toLocaleString('en-US')}  ${wallet.priorityClass}`);
console.log(`     block ${r.body.receipt.blockNumber}`);
  ...
  JSON.stringify(
    {
      ...
      balanceMinorUnit: subject.balance.toString(),
      priorityClass: wallet.priorityClass,
      depositorKey: wallet.depositorKey,
      claimId,
      merkleRoot: tree.root,
