 ...
 *   AND(OR('BankAMSP.peer','BankBMSP.peer'), 'CentralAuthorityMSP.peer')
 * so Central Authority's endorsement is a PRECONDITION OF COMMITMENT, not a
...
export class LifecycleContract extends Contract {
  ...
  async AppendEvent(
    ...
    //  only with Central Authority (and that institution's auditor).
    //
   ...
   *
   *  Run this as Central Authority and you get the payload. Run it as an officer
   ...
   * PENDING. It is not usable for a Board threshold until Central Authority
   ...
   *  Central Authority's prior approval (the Banking Act, s.15). Verity
   *  makes an existing approval a precondition the code checks, which is the
   *  same move it makes for Regulation A in AppendEvent.
   * ══════════════════════════════════════════════════════════════════════════
   */
  @Transaction()
