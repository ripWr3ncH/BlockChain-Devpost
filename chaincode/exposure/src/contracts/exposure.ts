 ...
 *       -> threshold decryption to Central Authority
...
const SUPERVISOR_MSP = 'CentralAuthorityMSP';
...
export class ExposureContract extends Contract {
  ...
  async RecordCeremony(
    ...
    const independents = participants.filter((p) => p !== 'CentralAuthority' && p !== SUPERVISOR_MSP);
    ...
    if (!key) {
      throw new Error(
        `AGGREGATION_PERIOD_CLOSED: no aggregation key published for ${period}; ` +
          'Central Authority opens a period before submissions are accepted',
      );
    }
    return key;
  }
