...
#  pointing at: Central Authority cannot revoke a bank's officer, and a bank
...
attempt() {
  ...
  out=$(curl -s --max-time 90 -X POST "$API/loans" \
    ...
    -d "$(jq -nc --arg l "$id" --arg z "$Z" '{commitmentId:$l,initialTier:"STANDARD",outstandingBand:"$1-10 M",groupToken:"G-0447",payloadHash:$z,originationDate:"2027-02-01"}')")
  local block
