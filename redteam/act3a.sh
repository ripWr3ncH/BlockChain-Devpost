...
#
#  The same query, run from two identities. Central Authority gets the payload.
...
OCA="$NET/organizations/ordererOrganizations/ord-bb.verity.local/orderers/orderer0.ord-bb.verity.local/msp/tlscacerts/tlsca.ord-bb.verity.local-cert.pem"
PEERS="--peerAddresses localhost:9051 --tlsRootCertFiles $NET/organizations/peerOrganizations/banka.verity.local/peers/peer0.banka.verity.local/tls/ca.crt --peerAddresses localhost:9071 --tlsRootCertFiles $NET/organizations/peerOrganizations/bb.verity.local/peers/peer0.bb.verity.local/tls/ca.crt"
...
PAYLOAD=$(jq -nc '{
  ...
  borrowerName: "Bank B Textiles Ltd (SYNTHETIC)",
  exactOutstandingMinorUnit: "10450000000000",
  justification: "Cash-flow disruption following buyer default; security revalued 12 Jan.",
  collateralValuation: "$62.4 M, regulator-enlisted valuer #114"
}')
...
echo "=================================================================="
as officer-rahim banka.verity.local BankAMSP 9051
peer chaincode invoke -o "$ORDERER" --ordererTLSHostnameOverride orderer0.ord-bb.verity.local \
  --tls --cafile "$OCA" -C commitment -n commitment $PEERS \
  -c "$(jq -nc --arg l "$LOAN" --arg h "$PHASH" '{Args:["LifecycleContract:OriginateLoan",$l,"STANDARD","$100-150 M","G-0447",$h,"2027-01-15"]}')" \
...
as officer-nasrin banka.verity.local BankAMSP 9051
peer chaincode invoke -o "$ORDERER" --ordererTLSHostnameOverride orderer0.ord-bb.verity.local \
  --tls --cafile "$OCA" -C commitment -n commitment $PEERS \
...
echo
echo "=================================================================="
echo " 1. Central Authority (supervisor) reads the payload"
echo "=================================================================="
as supervisor-1 bb.verity.local CentralAuthorityMSP 9071
...
echo "=================================================================="
as officer-shirin bankb.verity.local BankBMSP 9061
qry "$(jq -nc --arg l "$LOAN" '{Args:["LifecycleContract:ReadEventPayload",$l,"1"]}')" \
